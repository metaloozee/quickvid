...
export default async function handler(
  ...
  try {
    const response = await speechToTranscript(audioPath as string, openAIKey as string);
    ...
    return res.status(500).json({ error: "An error occurred during speech-to-text conversion" });
  }
}
